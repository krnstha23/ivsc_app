...
| 2026-04-20 | **Public landing (`app/page.tsx`):** Rebuilt to match `Frame.png` rhythm (hero, three cards, experience band, three-tier pricing, FAQ accordions, multi-column footer). **`ListeningMock`** is the only large UI mock, right column of `#experience`; hero stays **`/hero.png`**. **Pricing:** `PackageBundle` with `isActive` + `isFeatured` drives Standard / Priority / Instant amounts; **Rs 1200 / 1400 / 1600** fallback. Inline writing/report mocks removed; messaging folded into cards. **`components/landing-header.tsx`:** center nav (md+) to `#features`, `#experience`, `#pricing`, `/packages`. | -      |
...
- **Pricing source:** Featured bundle (`prisma.packageBundle.findFirst` where `isActive` and `isFeatured`); map to Standard (48h+), Priority (24–48h), Book for today; disclaimer when DB-backed vs fallback.
